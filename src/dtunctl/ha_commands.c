...
static int command_init(int argc, char **argv) {
  const char *config_path = dtunctl_option_value(argc, argv, "--config");
  const char *hub_id = dtunctl_option_value(argc, argv, "--hub-id");
  const char *dir = dtunctl_option_value(argc, argv, "--output-dir");
  const char *port_text = dtunctl_option_value(argc, argv, "--ha-port");
  const char *state_override = dtunctl_option_value(argc, argv, "--state-file");
  ...
  if (!dir)
    dir = DEFAULT_HA_DIR;
  if (dtunctl_path_join(key_path, sizeof(key_path), dir, "identity.key") < 0 ||
      dtunctl_path_join(ha_path, sizeof(ha_path), dir, "ha.conf") < 0)
    return 1;
  if (state_override)
...
static int command_rebuild(int argc, char **argv) {
  ...
  if (!dtunctl_has_option(argc, argv, "--force")) {
  ...
  if (!old_loaded && dtunctl_output_format(argc, argv, 0) != HA_FORMAT_JSON)
    fprintf(
        stderr,
...
static int load_state_arg(int argc, char **argv, dtun_ha_state_t *state,
                          const char **path) {
  *path = dtunctl_option_value(argc, argv, "--state-file");
  if (!*path)
...
static int command_invite_create(int argc, char **argv) {
  const char *hub_id = dtunctl_option_value(argc, argv, "--hub-id"),
             *weight_text = dtunctl_option_value(argc, argv, "--weight");
  const char *expires_text = dtunctl_option_value(argc, argv, "--expires"),
             *state_path,
             *key_path = dtunctl_option_value(argc, argv, "--identity-key");
  const char *bootstrap_text =
      dtunctl_option_value(argc, argv, "--bootstrap-address");
  const char *format = dtunctl_option_value(argc, argv, "--format");
  dtun_ha_state_t state;
  ...
  expires = dtunctl_duration_seconds(expires_text ? expires_text : "10m");
  if (weight < 1 || weight > 65535 || expires < 1) {
    fprintf(stderr, "invalid weight or expiry\n");
    return 2;
  }
  state_path = dtunctl_option_value(argc, argv, "--state-file");
  if (!state_path)
...
static int command_status(int argc, char **argv, int members) {
  ...
  uint32_t voters = 0;
  enum ha_output_format format = dtunctl_output_format(argc, argv, 0);
  const char *identity = dtunctl_option_value(argc, argv, "--identity-key");
...
static int command_invite_list(int argc, char **argv) {
  ...
  if (load_state_arg(argc, argv, &state, &path) < 0)
    return 1;
  int json = dtunctl_output_format(argc, argv, 0) == HA_FORMAT_JSON;
  if (json)
    putchar('[');
...
static int command_invite_revoke(int argc, char **argv) {
  const char *prefix = dtunctl_option_value(argc, argv, "--id-prefix"), *path;
  dtun_ha_state_t state;
  char id[33];
...
static int command_member(int argc, char **argv, const char *operation) {
  const char *id = dtunctl_option_value(argc, argv, "--hub-id"), *path;
  dtun_ha_state_t state;
  dtun_ha_member_t *m, *leader;
  int force = dtunctl_has_option(argc, argv, "--force");
  const char *identity = dtunctl_option_value(argc, argv, "--identity-key");
  dtun_ha_admin_reply_t reply;
  uint8_t action = 0;
  ...
  if (!strcmp(operation, "set-weight")) {
    const char *w = dtunctl_option_value(argc, argv, "--weight");
    ...
    snprintf(message, sizeof(message),
             ...
             "enable, or kick",
             operation);
    return dtunctl_ha_error(argc, argv, "ha.member", 2, message);
  }
  if (!strcmp(operation, "set-weight")) {
    state.manifest_version++;
    state.commit_index++;
...
static int command_failback(int argc, char **argv) {
  ...
  state.failback_force = dtunctl_has_option(argc, argv, "--force");
  state.commit_index++;
  if (dtun_ha_state_save(path, &state) < 0)
    return 1;
  if (dtunctl_output_format(argc, argv, 0) == HA_FORMAT_JSON)
...
static int command_join(int argc, char **argv) {
  ...
  if (!dir)
    dir = DEFAULT_HA_DIR;
  if (read_invite(&encoded,
                  dtunctl_has_option(argc, argv, "--invite-id-stdin")) < 0) {
  ...
  const char *state_option = dtunctl_option_value(argc, argv, "--state-file");
