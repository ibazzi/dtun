...
{
    size_t i, len;
    if (!hub_id || !(len = strlen(hub_id)) || len >= DTUN_HA_ID_LEN) return -1;
    for (i = 0; i < len; i++)
        if (!((hub_id[i] >= 'a' && hub_id[i] <= 'z') ||
              (hub_id[i] >= 'A' && hub_id[i] <= 'Z') ||
              (hub_id[i] >= '0' && hub_id[i] <= '9') ||
              hub_id[i] == '-' || hub_id[i] == '_')) return -1;
    return 0;
}

void dtun_ha_random_id(uint8_t *out, size_t len)
{
    if (!out || RAND_bytes(out, (int)len) != 1) abort();
}

int dtun_ha_make_parent_dirs(const char *path, mode_t mode)
{
    char copy[512], *p;
    if (!path || strlen(path) >= sizeof(copy)) return -ENAMETOOLONG;
    strcpy(copy, path);
    for (p = copy + 1; *p; p++) {
        if (*p != '/') continue;
        *p = '\0';
        if (mkdir(copy, mode) < 0 && errno != EEXIST) return -errno;
        *p = '/';
    }
    return 0;
}
...
int dtun_ha_state_load(const char *path, dtun_ha_state_t *state)
{
    ...
    if (file.version != DTUN_HA_STATE_VERSION) {
        fclose(fp); return -EPROTONOSUPPORT;
    }
    ...
    fclose(fp); *state = file.state; return 0;
}
...
int dtun_ha_state_lock(const char *path)
{
    ...
    if (!path || snprintf(lock_path, sizeof(lock_path), "%s.lock", path) >=
                 (int)sizeof(lock_path)) return -ENAMETOOLONG;
    if (dtun_ha_make_parent_dirs(lock_path, 0750) < 0) return -1;
    fd = open(lock_path, O_CREAT | O_RDWR | O_CLOEXEC, 0600);
    if (fd < 0) return -errno;
    if (flock(fd, LOCK_EX) < 0) { int error=-errno; close(fd); return error; }
    return fd;
}
                                      ...
                                      const char *hub_id)
{
    ...
    for (i = 0; i < state->member_count; i++)
        if (!strcmp(state->members[i].hub_id, hub_id)) return &state->members[i];
    return NULL;
}
...
int dtun_ha_identity_public(const char *private_path, uint8_t public_key[32])
{
    ...
    size_t len = 32;
    int result = key && EVP_PKEY_get_raw_public_key(key, public_key, &len) == 1 && len == 32 ? 0 : -1;
    EVP_PKEY_free(key); return result;
}

int dtun_ha_identity_generate(const char *private_path, uint8_t public_key[32])
{
    ...
    BUF_MEM *mem = NULL;
    int result = -1;
    if (!access(private_path, F_OK)) return -EEXIST;
    ctx = EVP_PKEY_CTX_new_id(EVP_PKEY_ED25519, NULL);
    if (!ctx || EVP_PKEY_keygen_init(ctx) != 1 || EVP_PKEY_keygen(ctx, &key) != 1) goto out;
    bio = BIO_new(BIO_s_mem());
    if (!bio || PEM_write_bio_PrivateKey(bio, key, NULL, NULL, 0, NULL, NULL) != 1) goto out;
    BIO_get_mem_ptr(bio, &mem);
    if (!mem || dtun_ha_atomic_write(private_path, mem->data, mem->length, 0600) < 0) goto out;
    result = dtun_ha_identity_public(private_path, public_key);
out:
    BIO_free(bio); EVP_PKEY_free(key); EVP_PKEY_CTX_free(ctx); return result;
}
...
static int sign_wire(const char *path, ha_invite_wire_t *wire)
{
    ...
    int ok = key && ctx && EVP_DigestSignInit(ctx, NULL, NULL, NULL, key) == 1 &&
        EVP_DigestSign(ctx, wire->signature, &len, (uint8_t *)wire,
                       offsetof(ha_invite_wire_t, signature)) == 1 && len == 64;
    EVP_MD_CTX_free(ctx); EVP_PKEY_free(key); return ok ? 0 : -1;
}
...
{
    for (; *text; text++) { if (*text == '+') *text = '-'; else if (*text == '/') *text = '_'; }
}
                          ...
                          char **encoded)
{
    ...
    memset(&wire, 0, sizeof(wire)); memcpy(wire.magic, HA_INVITE_MAGIC, 4); wire.version = 1;
    memcpy(wire.cluster_id, state->cluster_id, 16); memcpy(wire.invite_id, invite->id, 16);
    memcpy(wire.secret, secret, 32); wire.leader_addr = leader_addr.s_addr;
    wire.leader_port = htons(leader_port); wire.weight = htons(invite->weight);
    wire.expires_at = htobe64((uint64_t)invite->expires_at);
    memcpy(wire.hub_id, invite->hub_id, strnlen(invite->hub_id, sizeof(wire.hub_id)-1));
    if (dtun_ha_identity_public(private_key_path, wire.leader_key) < 0 ||
        sign_wire(private_key_path, &wire) < 0) { free(text); return -1; }
    strcpy(text, "dtun-ha1:");
    EVP_EncodeBlock((unsigned char *)text + 9, (const unsigned char *)&wire, sizeof(wire));
    base64url(text + 9); while (text[strlen(text)-1] == '=') text[strlen(text)-1] = '\0';
    *encoded = text; return 0;
}
                          ...
                          uint8_t leader_key[32])
{
    ...
    char *copy; size_t n, pad, decoded;
    if (!encoded || strncmp(encoded, "dtun-ha1:", 9)) return -EINVAL;
    n = strlen(encoded + 9); pad = (4 - n % 4) % 4;
    copy = calloc(1, n + pad + 1); if (!copy) return -ENOMEM;
    memcpy(copy, encoded + 9, n);
    for (size_t i=0;i<n;i++) { if(copy[i]=='-')copy[i]='+'; else if(copy[i]=='_')copy[i]='/'; }
    for (size_t i=0;i<pad;i++) copy[n+i]='=';
    decoded = (size_t)EVP_DecodeBlock(decoded_buffer, (unsigned char *)copy,
                                      (int)(n+pad));
    free(copy);
    if (decoded < pad || decoded-pad != sizeof(wire)) return -EINVAL;
    memcpy(&wire, decoded_buffer, sizeof(wire));
    if (memcmp(wire.magic, HA_INVITE_MAGIC, 4) || wire.version != 1 ||
        verify_wire(&wire) < 0) return -EINVAL;
    memset(invite, 0, sizeof(*invite)); memcpy(invite->id, wire.invite_id, 16);
    memcpy(invite->hub_id, wire.hub_id, strnlen(wire.hub_id, sizeof(invite->hub_id)-1));
    invite->weight = ntohs(wire.weight); invite->expires_at = (time_t)be64toh(wire.expires_at);
    memcpy(secret, wire.secret, 32); memcpy(cluster_id, wire.cluster_id, 16);
    memcpy(leader_key, wire.leader_key, 32); leader_addr->s_addr = wire.leader_addr;
    *leader_port = ntohs(wire.leader_port); return 0;
}

void dtun_ha_hex(const uint8_t *data, size_t len, char *out)
{
    static const char digits[] = "0123456789abcdef";
    for (size_t i=0;i<len;i++) { out[i*2]=digits[data[i]>>4]; out[i*2+1]=digits[data[i]&15]; }
    out[len*2]='\0';
}
