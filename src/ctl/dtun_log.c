...
static int parse_facility(const char *facility_str) {
    if (!facility_str) return LOG_DAEMON;
    if (!strcasecmp(facility_str, "daemon")) return LOG_DAEMON;
    if (!strcasecmp(facility_str, "user")) return LOG_USER;
    if (!strcasecmp(facility_str, "local0")) return LOG_LOCAL0;
    if (!strcasecmp(facility_str, "local1")) return LOG_LOCAL1;
    if (!strcasecmp(facility_str, "local2")) return LOG_LOCAL2;
    if (!strcasecmp(facility_str, "local3")) return LOG_LOCAL3;
    if (!strcasecmp(facility_str, "local4")) return LOG_LOCAL4;
    if (!strcasecmp(facility_str, "local5")) return LOG_LOCAL5;
    if (!strcasecmp(facility_str, "local6")) return LOG_LOCAL6;
    if (!strcasecmp(facility_str, "local7")) return LOG_LOCAL7;
    if (!strcasecmp(facility_str, "auth") || !strcasecmp(facility_str, "authpriv")) return LOG_AUTHPRIV;
    return LOG_DAEMON;
}
...
void dtun_vlog(int priority, const char *fmt, va_list ap) {
    ...
    FILE *stream = (priority == LOG_ERR || priority == LOG_WARNING ||
                    priority == LOG_EMERG || priority == LOG_ALERT ||
                    priority == LOG_CRIT) ? stderr : stdout;
    fprintf(stream, "%s\n", buf);
    fflush(stream);
