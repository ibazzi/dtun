...
static int add_attr(struct nlmsghdr *n, size_t maxlen, int type,
...
static struct rtattr *add_nested_attr(struct nlmsghdr *n, size_t maxlen,
                                      int type) {
  struct rtattr *rta =
      (struct rtattr *)(((char *)n) + NLMSG_ALIGN(n->nlmsg_len));
...
static void end_nested_attr(struct nlmsghdr *n, struct rtattr *rta) {
  rta->rta_len =
      (char *)(((char *)n) + NLMSG_ALIGN(n->nlmsg_len)) - (char *)rta;
}

static int genl_request(uint16_t family, uint8_t cmd, const void *attrs,
                        size_t attrs_len, void *reply_buf, size_t *reply_len) {
  if (genl_fd < 0)
    return -1;
  ...
  struct genlmsghdr *genl =
      (struct genlmsghdr *)(buf + sizeof(struct nlmsghdr));
  ...
  if (attrs && attrs_len > 0) {
    memcpy(buf + sizeof(struct nlmsghdr) + sizeof(struct genlmsghdr), attrs,
           attrs_len);
  }
  ...
  if (sendto(genl_fd, buf, nlh->nlmsg_len, 0, (struct sockaddr *)&sa,
             sizeof(sa)) < 0) {
    return -errno;
  }
...
static int parse_peer_status(const void *payload, size_t payload_len,
  ...
  while (RTA_OK(attr, len)) {
    ...
    case DTUN_A_IFINDEX:
      status->ifindex = *(uint32_t *)RTA_DATA(attr);
      break;
    case DTUN_A_TUNNEL_ID:
      status->tunnel_id = *(uint32_t *)RTA_DATA(attr);
      break;
    case DTUN_A_REMOTE_TUNNEL_ID:
      status->remote_tunnel_id = *(uint32_t *)RTA_DATA(attr);
      break;
    case DTUN_A_NODE_ID:
      memcpy(&status->node_id, RTA_DATA(attr), sizeof(status->node_id));
      break;
    case DTUN_A_RAW_ADDR:
      memcpy(&status->raw_addr.s_addr, RTA_DATA(attr), 4);
      break;
    case DTUN_A_RAW_VALIDATED_ADDR:
      memcpy(&status->raw_validated_addr.s_addr, RTA_DATA(attr), 4);
      break;
    case DTUN_A_UDP_ADDR:
    case DTUN_A_RENDEZVOUS_UDP_ADDR:
      memcpy(&status->udp_addr.s_addr, RTA_DATA(attr), 4);
      break;
    case DTUN_A_UDP_PORT:
    case DTUN_A_RENDEZVOUS_UDP_PORT:
      status->udp_port = ntohs(*(uint16_t *)RTA_DATA(attr));
      break;
    case DTUN_A_DIRECT_UDP_ADDR:
      memcpy(&status->direct_udp_addr.s_addr, RTA_DATA(attr), 4);
      break;
    case DTUN_A_DIRECT_UDP_PORT:
      status->direct_udp_port = ntohs(*(uint16_t *)RTA_DATA(attr));
      break;
    case DTUN_A_CANDIDATE_GENERATION:
      memcpy(&status->candidate_generation, RTA_DATA(attr),
             sizeof(status->candidate_generation));
      break;
    case DTUN_A_DYNAMIC_RAW:
      status->dynamic_raw = *(uint8_t *)RTA_DATA(attr);
      break;
    case DTUN_A_RAW_UP:
      status->raw_up = *(uint8_t *)RTA_DATA(attr);
      break;
    case DTUN_A_UDP_UP:
      status->udp_up = *(uint8_t *)RTA_DATA(attr);
      break;
...
static int genl_peer_dump(uint32_t ifindex, dtun_nl_peer_status_t **statuses,
  ...
  *status_count = 0;
  ...
  if (sendto(genl_fd, request, nlh->nlmsg_len, 0, (struct sockaddr *)&sa,
             sizeof(sa)) < 0)
    return -errno;
  ...
  for (;;) {
    ...
    for (msg = (struct nlmsghdr *)buffer; NLMSG_OK(msg, remaining);
      ...
      if (msg->nlmsg_type != dtun_genl_family_id)
        continue;
      err = parse_peer_status(NLMSG_DATA(msg), msg->nlmsg_len - NLMSG_HDRLEN,
                              &status);
      if (err < 0) {
        free(items);
        return err;
...
static uint16_t genl_resolve_family(const char *name) {
  ...
  int res = genl_request(GENL_ID_CTRL, CTRL_CMD_GETFAMILY, attrs,
                         RTA_ALIGN(rta->rta_len), reply, &reply_len);
  if (res < 0)
    return 0;

  struct genlmsghdr *gmsg = (struct genlmsghdr *)reply;
  struct rtattr *attr =
      (struct rtattr *)((char *)gmsg + sizeof(struct genlmsghdr));
...
int dtun_link_create(const char *ifname, struct in_addr local_addr,
  ...
  if (sendto(fd, buf, n->nlmsg_len, 0, (struct sockaddr *)&sa, sizeof(sa)) <
      0) {
    close(fd);
    return -errno;
  }
...
int dtun_nl_peer_add(const dtun_nl_peer_info_t *peer) {
  ...
  off += pack_dtun_attr(attrs + off, sizeof(attrs) - off, DTUN_A_IFINDEX,
                        &peer->ifindex, 4);
  off += pack_dtun_attr(attrs + off, sizeof(attrs) - off, DTUN_A_TUNNEL_ID,
                        &peer->tunnel_id, 4);
  off += pack_dtun_attr(attrs + off, sizeof(attrs) - off,
                        DTUN_A_REMOTE_TUNNEL_ID, &peer->remote_tunnel_id, 4);
  off += pack_dtun_attr(attrs + off, sizeof(attrs) - off, DTUN_A_NODE_ID,
                        &peer->node_id, 8);
  off += pack_dtun_attr(attrs + off, sizeof(attrs) - off, DTUN_A_RAW_ADDR,
                        &peer->raw_addr.s_addr, 4);
  off += pack_dtun_attr(attrs + off, sizeof(attrs) - off, DTUN_A_UDP_ADDR,
                        &peer->udp_addr.s_addr, 4);
  uint16_t port = htons(peer->udp_port);
  off += pack_dtun_attr(attrs + off, sizeof(attrs) - off, DTUN_A_UDP_PORT,
                        &port, 2);

  if (peer->has_key) {
    off += pack_dtun_attr(attrs + off, sizeof(attrs) - off, DTUN_A_KEY,
                          peer->key, 32);
  }
  uint8_t dynamic_raw = peer->dynamic_raw ? 1 : 0;
  if (peer->has_dynamic_raw)
    off += pack_dtun_attr(attrs + off, sizeof(attrs) - off, DTUN_A_DYNAMIC_RAW,
                          &dynamic_raw, 1);
  if (peer->has_generation)
    off += pack_dtun_attr(
        attrs + off, sizeof(attrs) - off, DTUN_A_CANDIDATE_GENERATION,
        &peer->candidate_generation, sizeof(peer->candidate_generation));

  return genl_request(dtun_genl_family_id, DTUN_CMD_PEER_ADD, attrs, off, NULL,
                      NULL);
}
...
int dtun_nl_peer_set(const dtun_nl_peer_info_t *peer) {
  ...
  return genl_request(dtun_genl_family_id, DTUN_CMD_PEER_SET, attrs, off, NULL,
                      NULL);
}
...
int dtun_nl_peer_del(uint32_t ifindex, uint32_t tunnel_id) {
  ...
  return genl_request(dtun_genl_family_id, DTUN_CMD_PEER_DEL, attrs, off, NULL,
                      NULL);
}
...
int dtun_nl_peer_get(uint32_t ifindex, uint32_t tunnel_id,
  ...
  char reply[512];
  size_t reply_len = sizeof(reply);
  int res = genl_request(dtun_genl_family_id, DTUN_CMD_PEER_GET, attrs, off,
                         reply, &reply_len);
  if (res < 0)
    return res;
...
int dtun_nl_rebind(uint32_t ifindex) {
  ...
  return genl_request(dtun_genl_family_id, DTUN_CMD_REBIND, attrs, off, NULL,
                      NULL);
}
...
int dtun_nl_hub_set(uint32_t ifindex, struct in_addr hub_addr,
  ...
  if (err < 0)
    return err;
  if (!ifindex || !hub_addr.s_addr || !hub_port)
    return -EINVAL;
  off += pack_dtun_attr(attrs + off, sizeof(attrs) - off, DTUN_A_IFINDEX,
                        &ifindex, sizeof(ifindex));
...
int dtun_nl_route_add(uint32_t ifindex, uint32_t tunnel_id,
  ...
  off += pack_dtun_attr(attrs + off, sizeof(attrs) - off, DTUN_A_TUNNEL_ID,
                        &tunnel_id, 4);
  off += pack_dtun_attr(attrs + off, sizeof(attrs) - off, DTUN_A_PREFIX,
                        &prefix.s_addr, 4);
  off += pack_dtun_attr(attrs + off, sizeof(attrs) - off, DTUN_A_PREFIX_LEN,
                        &prefix_len, 1);

  return genl_request(dtun_genl_family_id, DTUN_CMD_ROUTE_ADD, attrs, off, NULL,
                      NULL);
}
...
int dtun_nl_route_del(uint32_t ifindex, uint32_t tunnel_id,
  ...
  return genl_request(dtun_genl_family_id, DTUN_CMD_ROUTE_DEL, attrs, off, NULL,
                      NULL);
}
...
int dtun_module_ensure_loaded(void) {
  ...
  dtun_log_info(
      "[dtund] Kernel module 'dtun' is not loaded, attempting to load...");

  int ret = system("modprobe dtun 2>/dev/null");
  if (ret != 0 || access("/sys/module/dtun", F_OK) != 0) {
    ret = system(
        "insmod ./build/dtun.ko 2>/dev/null || insmod ./dtun.ko 2>/dev/null");
    (void)ret;
  }
  ...
  dtun_log_err("[dtund] Error: Failed to load kernel module 'dtun'. Please "
               "ensure dtun.ko is installed or present.");
  return -1;
}
