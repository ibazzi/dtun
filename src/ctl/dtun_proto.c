...
ssize_t dtrg_pack_ack(const uint8_t *key, uint64_t node_id, uint32_t tunnel_id, uint32_t remote_tunnel_id,
    ...
    size_t body_len = 4 + 1 + 8 + 4 + 4 + 4 + 1 + 2 + 16 + 16 + 8;
    if (max_len < body_len + DTRG_TAG_LEN) return -1;
    ...
    *p++ = prefix_len;
    ...
    memcpy(p, nonce, 16); p += 16;
    memcpy(p, lease_token, 16); p += 16;
    uint64_t epoch_be = hton64(epoch); memcpy(p, &epoch_be, 8); p += 8;
...
ssize_t dtrg_pack_refresh(const uint8_t *key, uint64_t node_id,
    ...
    uint64_t value;
    uint16_t offset_be;
    if (!lease_token || max_len < body_len + DTRG_TAG_LEN) return -1;
    memcpy(p, DTRG_MAGIC, 4); p += 4;
    *p++ = DTRG_REFRESH;
    value = hton64(node_id); memcpy(p, &value, 8); p += 8;
    memcpy(p, lease_token, 16); p += 16;
    value = hton64(counter); memcpy(p, &value, 8); p += 8;
    value = hton64(epoch); memcpy(p, &value, 8); p += 8;
    offset_be = htons(offset); memcpy(p, &offset_be, 2); p += 2;
    dtrg_hmac(key, DTRG_KEY_LEN, out, body_len, p);
...
ssize_t dtrg_pack_refresh_ack(const uint8_t *key, uint64_t node_id,
                              ...
                              uint8_t *out, size_t max_len) {
    const size_t peer_wire_len = 8 + 4 + 4 + 4 + 4 + 4 + 2 + 8 + 1;
    size_t body_len = 4 + 1 + 8 + 16 + 8 + 8 + 4 + 2 + 1 + 2 + 2 +
                      (size_t)peer_count * peer_wire_len;
    uint8_t *p = out;
    uint64_t value;
    uint16_t short_value;
    uint16_t i;
    if (!lease_token || peer_count > DTRG_MAX_SYNC_PEERS ||
    ...
    *p++ = DTRG_REFRESH_ACK;
    ...
    value = hton64(counter); memcpy(p, &value, 8); p += 8;
    value = hton64(epoch); memcpy(p, &value, 8); p += 8;
    memcpy(p, &observed_addr.s_addr, 4); p += 4;
    short_value = htons(observed_port); memcpy(p, &short_value, 2); p += 2;
    *p++ = flags;
    short_value = htons(next_offset); memcpy(p, &short_value, 2); p += 2;
    short_value = htons(peer_count); memcpy(p, &short_value, 2); p += 2;
    for (i = 0; i < peer_count; i++) {
        value = hton64(peers[i].node_id); memcpy(p, &value, 8); p += 8;
        uint32_t word = htonl(peers[i].tunnel_id); memcpy(p, &word, 4); p += 4;
        word = htonl(peers[i].remote_tunnel_id); memcpy(p, &word, 4); p += 4;
        memcpy(p, &peers[i].address.s_addr, 4); p += 4;
        memcpy(p, &peers[i].raw.s_addr, 4); p += 4;
        memcpy(p, &peers[i].udp_addr.s_addr, 4); p += 4;
        short_value = htons(peers[i].udp_port); memcpy(p, &short_value, 2); p += 2;
        value = hton64(peers[i].generation); memcpy(p, &value, 8); p += 8;
        *p++ = peers[i].flags;
    }
    dtrg_hmac(key, DTRG_KEY_LEN, out, body_len, p);
    return (ssize_t)(body_len + DTRG_TAG_LEN);
...
int dtrg_parse(const uint8_t *key, const uint8_t *pkt, size_t pkt_len, dtrg_msg_t *msg) {
    ...
    if (pkt_len < 4 + 1 + DTRG_TAG_LEN) return -1;
    ...
    if (kind == DTRG_INIT) {
        if (body_len != 4 + 1 + 8 + 4 + 1 + 4 + 16) return -5;
    ...
    if (kind == DTRG_CHALLENGE || kind == DTRG_CONFIRM) {
        if (body_len != 4 + 1 + 8 + 4 + 1 + 4 + 16 + 32) return -5;
        uint64_t nid; memcpy(&nid, p, 8); msg->node_id = ntoh64(nid); p += 8;
        memcpy(&msg->address.s_addr, p, 4); p += 4;
        msg->prefix_len = *p++;
        memcpy(&msg->raw.s_addr, p, 4); p += 4;
    ...
    if (kind == DTRG_ACK) {
        if (body_len != 4 + 1 + 8 + 4 + 4 + 4 + 1 + 2 + 16 + 16 + 8) return -5;
        uint64_t nid; memcpy(&nid, p, 8); msg->node_id = ntoh64(nid); p += 8;
        ...
        memcpy(msg->nonce, p, 16); p += 16;
        memcpy(msg->lease_token, p, 16); p += 16;
        uint64_t epoch; memcpy(&epoch, p, 8); msg->epoch = ntoh64(epoch); p += 8;
        return 0;
    }
    ...
    if (kind == DTRG_REFRESH) {
        uint64_t value;
        uint16_t offset;
        if (body_len != 4 + 1 + 8 + 16 + 8 + 8 + 2) return -5;
        memcpy(&value, p, 8); msg->node_id = ntoh64(value); p += 8;
        memcpy(msg->lease_token, p, 16); p += 16;
        memcpy(&value, p, 8); msg->counter = ntoh64(value); p += 8;
        memcpy(&value, p, 8); msg->epoch = ntoh64(value); p += 8;
        memcpy(&offset, p, 2); msg->offset = ntohs(offset); p += 2;
        return 0;
    }
    ...
    if (kind == DTRG_REFRESH_ACK) {
        ...
        memcpy(&value, p, 8); msg->counter = ntoh64(value); p += 8;
        memcpy(&value, p, 8); msg->epoch = ntoh64(value); p += 8;
        memcpy(&msg->observed_addr.s_addr, p, 4); p += 4;
        memcpy(&short_value, p, 2); msg->observed_port = ntohs(short_value); p += 2;
        msg->flags = *p++;
        memcpy(&short_value, p, 2); msg->offset = ntohs(short_value); p += 2;
        memcpy(&count, p, 2); count = ntohs(count); p += 2;
        if (count > DTRG_MAX_SYNC_PEERS ||
            body_len != header_len + (size_t)count * peer_wire_len) return -5;
        msg->peers = count ? calloc(count, sizeof(*msg->peers)) : NULL;
        if (count && !msg->peers) return -7;
        msg->peer_count = count;
        for (i = 0; i < count; i++) {
            uint32_t word;
            memcpy(&value, p, 8); msg->peers[i].node_id = ntoh64(value); p += 8;
            memcpy(&word, p, 4); msg->peers[i].tunnel_id = ntohl(word); p += 4;
            memcpy(&word, p, 4); msg->peers[i].remote_tunnel_id = ntohl(word); p += 4;
            memcpy(&msg->peers[i].address.s_addr, p, 4); p += 4;
            memcpy(&msg->peers[i].raw.s_addr, p, 4); p += 4;
            memcpy(&msg->peers[i].udp_addr.s_addr, p, 4); p += 4;
            memcpy(&short_value, p, 2); msg->peers[i].udp_port = ntohs(short_value); p += 2;
            memcpy(&value, p, 8); msg->peers[i].generation = ntoh64(value); p += 8;
            msg->peers[i].flags = *p++;
        }
        return 0;
    }
