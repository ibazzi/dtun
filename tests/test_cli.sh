...
expect_rc 2 "$CTL" peer-get --ifindex 1 --tunnel-id 1 --format yaml
expect_rc 2 "$CTL" route-add --format json --ifindex 1 --tunnel-id 1 --prefix 10.0.0.0/24

set +e
json=$($CTL peer-get --format=json --ifindex 4294967295 --tunnel-id 1 2>/dev/null)
rc=$?
set -e
[ "$rc" -ne 0 ]
