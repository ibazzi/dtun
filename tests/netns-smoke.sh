...
[ -e "$ROOT/dtun.ko" ] || fail "build dtun.ko first"
...
$IP netns exec dtun-a "$CTL" peer-add --ifindex "$IFA" --tunnel-id 100 --node-id 2 --raw 172.31.0.2 --udp 172.31.0.2:49000 --key "$KEY"
$IP netns exec dtun-b "$CTL" peer-add --ifindex "$IFB" --tunnel-id 100 --node-id 1 --raw 172.31.0.1 --udp 172.31.0.1:49000 --key "$KEY"
...
$IP netns exec dtun-a "$CTL" peer-del --ifindex "$IFA" --tunnel-id 101
...
$IP netns exec dtun-a "$CTL" peer-set --ifindex "$IFA" --tunnel-id 100 --raw 0.0.0.0 --udp 172.31.0.2:49000
$IP netns exec dtun-b "$CTL" peer-set --ifindex "$IFB" --tunnel-id 100 --raw 0.0.0.0 --udp 172.31.0.1:49000
sleep 6
if [ "${DEBUG:-0}" = 1 ]; then
	$IP netns exec dtun-a "$CTL" peer-get --ifindex "$IFA" --tunnel-id 100
	$IP netns exec dtun-b "$CTL" peer-get --ifindex "$IFB" --tunnel-id 100
fi
$IP netns exec dtun-a ping -c 3 -W 1 10.20.0.2
$IP netns exec dtun-a "$CTL" peer-set --ifindex "$IFA" --tunnel-id 100 --raw 172.31.0.2 --udp 172.31.0.2:49000
$IP netns exec dtun-b "$CTL" peer-set --ifindex "$IFB" --tunnel-id 100 --raw 172.31.0.1 --udp 172.31.0.1:49000
sleep 6
$IP netns exec dtun-a ping -c 3 -W 1 10.20.0.2
...
$IP netns exec dtun-a "$CTL" peer-set --ifindex "$IFA" --tunnel-id 100 --udp 172.31.0.2:49001
