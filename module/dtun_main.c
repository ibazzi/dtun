...
static struct dtun_peer *dtun_peer_get_by_id(struct dtun_dev *d,
                                             u32 tunnel_id) {
...
static struct dtun_peer *dtun_peer_get_by_node(struct dtun_dev *d,
                                               u64 node_id) {
  struct dtun_peer *peer;
...
static int dtun_ingress(struct dtun_dev *d, struct sk_buff *skb, __be32 src,
  ...
  skb_dst_drop(skb);
  skb->protocol =
      (skb->data[0] >> 4) == 6 ? htons(ETH_P_IPV6) : htons(ETH_P_IP);
...
static int dtun_raw_rcv(struct sk_buff *skb) {
  ...
  list_for_each_entry_rcu(d, &dtun_devices, global_list) {
    if (hdr->dst_node == cpu_to_be64(d->node_id)) {
      ret = dtun_ingress(d, skb, ip_hdr(skb)->saddr, 0, DTUN_TRANSPORT_RAW);
      rcu_read_unlock();
      return ret;
    }
  }
...
static struct dtun_peer *dtun_route_peer(struct dtun_dev *d,
                                         struct sk_buff *skb) {
 ...
 */
static int dtun_peer_snapshot(struct dtun_dev *d,
                              struct dtun_peer ***snapshot) {
...
static int dtun_send_path(struct dtun_dev *d, struct dtun_peer *peer, u8 type,
  ...
  skb = alloc_skb(LL_MAX_HEADER + sizeof(struct iphdr) + sizeof(struct udphdr) +
                      frame_len,
                  GFP_ATOMIC);
  if (!skb)
    return -ENOMEM;
  ...
  err = dtun_tag(peer, hdr, (u8 *)hdr + sizeof(*hdr), payload_len, hdr->tag);
  spin_unlock_bh(&peer->state_lock);
  if (err)
    goto out_free;
  ...
  if (transport == DTUN_TRANSPORT_RAW) {
    ...
    iptunnel_xmit(NULL, rt, skb, source, addr, DTUN_IPPROTO, 0, IPDEFTTL, 0,
                  false, 0);
#else
    iptunnel_xmit(NULL, rt, skb, source, addr, DTUN_IPPROTO, 0, IPDEFTTL, 0,
                  false);
    ...
    rt = ip_route_output_ports(dev_net(d->dev), &fl4, NULL, addr, d->local_addr,
...
static netdev_tx_t dtun_xmit(struct sk_buff *skb, struct net_device *dev) {
  ...
  if (multicast) {
    for (i = 0; i < peer_count; i++) {
      err = dtun_send(d, peers[i], DTUN_FRAME_DATA, payload, skb->len);
      if (err)
...
static void dtun_probe_work(struct work_struct *work) {
  ...
  for (i = 0; i < used; i++) {
    ...
    udp_addr = peer->direct_udp_addr ? peer->direct_udp_addr : peer->udp_addr;
...
struct dtun_peer *dtun_peer_by_id(struct dtun_dev *d, u32 tunnel_id) {
  ...
  list_for_each_entry(peer, &d->peers,
                      list) if (peer->tunnel_id == tunnel_id) return peer;
  return NULL;
}
...
struct dtun_peer *dtun_peer_by_node(struct dtun_dev *d, u64 node_id) {
  ...
  list_for_each_entry(peer, &d->peers,
                      list) if (peer->node_id == node_id) return peer;
  return NULL;
}
...
int dtun_add_prefix(struct dtun_peer *peer, __be32 addr, u8 len) {
  ...
  if (len > 32)
    return -EINVAL;
  list_for_each_entry(prefix, &peer->prefixes,
                      list) if (prefix->addr == addr &&
                                prefix->len == len) return -EEXIST;
