  ...
  for (uint32_t i = 0; i < r->health_count; i++)
    if (!strcmp(r->health[i].hub_id, m->hub_id))
      return r->health[i].reachable &&
             now - r->health[i].last_heartbeat <= r->failover_timeout;
  return 0;
}
...
static int has_quorum(const dtund_ha_runtime_t *r, time_t now) {
  ...
  for (uint32_t i = 0; i < r->persistent.member_count; i++)
    if (r->persistent.members[i].enabled &&
      ...
      voters++;
      healthy += member_healthy(r, i, now);
    }
...
int dtund_ha_runtime_init(dtund_ha_runtime_t *r, const dtun_config_t *c,
  ...
  memset(r, 0, sizeof(*r));
  r->persistent = *s;
  r->failover_timeout = c->failover_timeout;
...
int dtund_ha_tick(dtund_ha_runtime_t *r, time_t now) {
  ...
  if (r->phase == DTUND_HA_STANDBY &&
      now - r->leader_last_seen >= r->failover_timeout) {
    if (r->persistent.member_count == 2)
      promote(r, now);
    else if (r->persistent.member_count >= 3 && has_quorum(r, now) &&
             local_is_best_candidate(r, now))
      promote(r, now);
  } else if ((r->phase == DTUND_HA_PRIMARY_ACTIVE ||
              r->phase == DTUND_HA_PRIMARY_PROBATION) &&
             r->persistent.member_count >= 3 && !has_quorum(r, now)) {
    r->phase = DTUND_HA_STANDBY;
    r->phase_since = now;
  } else if (r->phase == DTUND_HA_BACKUP_HOLDDOWN &&
             now - r->phase_since >= r->min_backup_active_time) {
